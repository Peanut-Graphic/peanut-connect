...
= 3.11.3 =
* Security: event banner (/banner/show) hardened against site-wide injection — CSS sanitised against url() exfiltration / @import / expression() / tag breakout, HTML restricted to a tight banner-only allowlist, position constrained + JSON-encoded for its inline-script context.
